document.addEventListener('DOMContentLoaded', () => {
    ...
    backToMenuBtn.addEventListener('click', () => {
        saveProgress(); // Guardar progreso antes de salir por si acaso
        clearInterval(timerInterval);
        hideAllAreas();
        document.getElementById('controls-section').style.display = 'flex';
        backToMenuBtn.style.display = 'none';
        
        // Limpiar vistas de revisiones y test
    ...
    function renderDifficultQuestions() {
        ...
        sortedQs.forEach((q, index) => {
            ...
            html += `
                <div class="difficult-question-item ${markClass}">
                    <div class="difficult-question-meta" style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;">
                        <div>
                            Test: ${q.test.replace('.json', '')} | Pregunta ${q.qIndex + 1} | <strong style="color: ${q.mark === 'red' ? '#dc3545' : '#ffc107'}">${markLabel}</strong>
                        </div>
                        <button class="mark-known-btn" data-test="${q.test}" data-qindex="${q.qIndex}" title="Marcar como sabida (Azul) y quitar de la lista">✓ Ya me la sé (Azul)</button>
                    </div>
                    <p style="font-weight: bold; margin-bottom: 1rem; margin-top: 0.5rem;">${q.questionText}</p>
        ...
        // Añadir event listeners para los botones "Ya me la sé"
        const markBtns = difficultQuestionsArea.querySelectorAll('.mark-known-btn');
        markBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                const test = e.target.getAttribute('data-test');
                const qIndex = parseInt(e.target.getAttribute('data-qindex'), 10);
                
                // Efecto visual de cambio a azul antes de desaparecer
                const item = e.target.closest('.difficult-question-item');
                item.style.transition = 'all 0.4s ease';
                item.style.borderLeftColor = '#007bff'; // Cambiar borde a azul
                item.style.opacity = '0'; // Desvanecer
                item.style.transform = 'translateX(20px)'; // Deslizar
                
                setTimeout(() => {
                    removeDifficultQuestion(test, qIndex);
